...
const losuData = {
  ...
  driverNumber: 1,
  destinyNumber: 2,
  driverAddedToGrid: false,
  destinyAddedToGrid: true,
  grid: {
    topRow: ["", "99", "2"],
    bottomRow: ["", "11", "6"],
    middleRow: ["3", "", ""],
...
export default function LosuPage() {
  const [dob, setDob] = useState(losuData.dob);
  const [losuResult, setLosuResult] = useState(losuData);
  const [personalYearResult, setPersonalYearResult] = useState(personalYearData);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [message, setMessage] = useState("");
  const countEntries = Object.entries(losuResult.counts);
  const summaryCards = [
    {
      label: "Date of Birth",
    ...
    <PageLayout title="Lo Shu Details" icon="🔢">
      ...
      <div className="mx-auto max-w-4xl">
        <section className="rounded-xl border border-white/10 bg-[#0f1535] p-3 shadow-lg">
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-[0.85fr_1fr]">
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-0.5 max-w-2xl text-sm leading-5 text-gray-400">
                  Client grid summary prepared from the shared birth-date
                  calculation.
                </p>
              </div>
              ...
              >
                ...
                <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    placeholder="10-03-1996"
                    className="min-w-0 flex-1 rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-sm text-white outline-none transition focus:border-[#d8a84a]/70"
                  />
                  <button
                    type="submit"
