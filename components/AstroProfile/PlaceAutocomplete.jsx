...
import { searchLocation } from "@/services/astroProfile.service";
      ...
      {isOpen && (
        <div className="absolute z-40 mt-2 max-h-64 w-full overflow-y-auto rounded-xl border border-[#eadcae] bg-white p-1 text-[#211704] shadow-[0_18px_42px_rgba(70,48,8,0.16)]">
          {options.map((place, index) => (
            ...
            >
              <span className="block font-semibold">{place.placeName}</span>
              {place.timezoneId && (
                <span className="text-xs text-[#6f5930]">{place.timezoneId}</span>
              )}
            </button>
          ))}
